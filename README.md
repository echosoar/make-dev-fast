...
### find 命令：在当前目录下搜索内容和文件名

根据关键词，递归搜索当前目录下所有文件中匹配的内容，同时也会搜索文件名中包含该关键词的文件。
...
# 限制每类结果的数量（达到后即停止搜索）
$ dev find "xxx" --limit=20

# 同时搜索被忽略的目录/文件（node_modules 和以 . 开头的目录/文件）
$ dev find "xxx" --all
```

包含如下功能：
+ 内容搜索：展示匹配到的文件名、行号以及对应内容（包含匹配关键词前后的字符作为上下文）。
+ 文件名搜索：展示文件名中包含关键词的文件。
+ 数量控制：默认最多展示 100 条结果（文件内容最多 50 个、文件名最多 50 个），同时会展示总共匹配到多少个文件内容和文件名。
+ `--limit` 参数：覆盖默认的展示数量；当文件内容和文件名都达到该数量后，会停止继续搜索。
+ `--all` 参数：默认会过滤 `node_modules` 目录以及以 `.` 开头的目录和文件，使用 `--all` 可以搜索这些被忽略的内容。
+ 自动跳过二进制文件，避免输出乱码。

### cmd/run 命令：列出并运行项目内可执行的命令

执行后会自动分析当前项目，列出所有可执行的命令，并通过可搜索的列表选择后直接运行（在当前终端中交互式运行，支持本地服务、构建等长任务）。
...
命令列表来源于：
+ `package.json` 中的 `scripts`（生成 `npm run <name>`）
+ `Cargo.toml` 中的 `name`（生成 `cargo build --release -p <name>`）
+ Xcode 工程（`*.xcodeproj/project.pbxproj`，生成 `xcodebuild` 命令）
+ `README.md`（包含大写版本）中的 ` ```bash ` / ` ```sh ` / ` ```shell ` 代码块，代码块上方的 `#` 注释会作为命令名称
